...
6. После создания новой ветки `backup/*` workflow `.github/workflows/bot-state-backup.yml` автоматически оставляет ровно три последних проверенных обычных backup-ветки. Он обрабатывает события `create` и создание ветки через `push`; ручной запуск и ежедневный schedule выполняют ту же ротацию как страховочный путь.
...
- после создания нового `backup/*` остаются ровно три последние проверенные ветки; если интеграция не сформировала событие создания, тот же inventory проверяется при ручном запуске и ежедневном schedule;
