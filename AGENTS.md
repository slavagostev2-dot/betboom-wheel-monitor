...
- Удаление колеса из общего списка выполняет только автоматическая BetBoom API-проверка; администратор может только указать или изменить время.
- Старые callback общего удаления сохраняются как безопасная совместимость: они отвечают, что действие отключено, и не меняют общее состояние.
