...
- Автоучастие: `auto_participation_worker.py` выполняет первую event-попытку; `auto_participation_recovery.py` независимо пересканирует свежие публикации текущего `public_sources.txt`, сверяет ссылки с BetBoom API и восстанавливает потерянные active/event записи; `betboom_participation_browser.py` является устойчивым Playwright fallback фактического участия. После нового подтверждённого recovery-успеха `auto_participation_bot_sync.py` ставит только публичный pending-маркер в `state.json`, а `auto_participation_owner_sync.py` внутри единственного живого Control Center выполняет личную отметку владельца через штатный `mark_personal_participation`, рейтинг и успешное Telegram-уведомление.
...
- применённая в текущем цикле личная отметка подавляет адресное напоминание
  по точному поколению события, даже если локальная копия encrypted state ещё
  не успела обновиться; отметка предыдущего поколения напоминание не подавляет;
- колесо с наступившим deadline либо terminal API/lifecycle-статусом не
  показывается в активном списке ни одному пользователю; личное участие или его
  отсутствие не создаёт дополнительного UI grace-периода;
...
- `source_rating_epoch_day` имеет одно production-значение во всех loaders;
  загрузка статистики не должна переключать epoch туда-обратно и стирать
  `wheel_posts`. Однократное восстановление находок разрешено только из
  сохранённых уникальных `recent_post_keys` с их `seen_at` после текущего epoch.
...
- Только новый фактический успех browser/recovery получает `bot_success_pending_at`; статус `already_marked_participating` и исторические успешные события не создают ретроспективное успешное уведомление.
- Pending-успех потребляет только единственный живой Control Center: для владельца вызывается тот же `mark_personal_participation`, что у ручной кнопки «Участвую», поэтому используется стандартный `record_personal_vote`, HMAC-псевдоним, вес владельца 5 очков каждому уникальному источнику и идемпотентность одного пользователя на одно событие.
- После успешной личной отметки Control Center отправляет владельцу отдельное Telegram-сообщение «Автоучастие подтверждено» и сохраняет event-scoped completion в encrypted state; повторный цикл не создаёт второй голос или повторное успешное уведомление.
- Workflow автоучастия не записывает `bot_private_state.enc.json`: encrypted personal state остаётся ответственностью Control Center.
...
### Разведка и маршрутизация новых источников

- Голое `@упоминание` или ссылка `t.me` не являются кандидатом: рядом должен
  присутствовать тематический признак колёс/акций, ставок, стримов,
  киберспорта или поддерживаемых игровых дисциплин либо такой признак должен
  быть в username самого канала.
- Username, заканчивающийся на `bot` без учёта регистра, всегда исключается из
  разведки и автоматической маршрутизации: Telegram требует такой суффикс у
  ботов.
- Разведка никогда сама не добавляет кандидатов ни в основной, ни в ночной
  inventory. Она только показывает результаты администратору; изменение
  режима выполняется явной административной командой.
- `source_tier_maintenance.py` работает только как аудит: он перечисляет
  источники, которые можно рассмотреть для ночного режима, но не меняет
  `public_sources.txt` и `source_catalog.txt`.
- Ночной список пополняется только администратором. `nightly_discovery.py`
  сканирует уже одобренный список и остаётся единственным автоматическим
  владельцем tier-файлов: он может только повысить источник с подтверждённым
  активным колесом из ночного режима в основной.
- Успешное завершение ночного discovery сразу запускает transport smoke и
  перестроение source registry. Это обязательная связь, потому что runtime
  commit ночного состояния содержит `[skip ci]` и сам по себе не создаёт
  обычные push-runs.
- Health оценивает полноту intelligence по `known_sources` конкретного запуска,
  а inventory — независимо, через discovery и полный transport smoke.
- Ночной кандидат переходит в основную проверку только после подтверждения
  реально активного колеса по странице/API. Одного упоминания, старой ссылки
  или Telegram-времени для повышения недостаточно.
...
- Один authoritative файл не коммитится автоматическими workflow из разных
  ...
  изменяет только nightly discovery в группе `bb-vg-source-catalog-writer`;
  source-tier maintenance сохраняет только диагностический аудит.
