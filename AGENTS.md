...
- Реферальное ограничение определяется по тексту публикации в `wheel_publications_v2.py`. Пометка «Колесо только для рефералов» обязательна в первичном уведомлении, но не запрещает автоматические попытки участия. Если хотя бы один из двух итогов реферального колеса отрицательный, итоговое Telegram-сообщение не отправляется; подтверждённый успех обоих аккаунтов может быть отправлен как один общий результат.
...
- Надёжность Telegram-доставки: `notification_integrity_v2.py` хранит HMAC-ledger и локальный claim, а `notification_remote_checkpoint.py` до внешней отправки фиксирует claim в `main` через Contents API и запускает recovery-автоучастие для новых wheel-event. Каждое уведомление о колесе обязано содержать маршрут «Активные колёса» и «Главное меню».
