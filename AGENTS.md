...
- Username, заканчивающийся на `bot` без учёта регистра, всегда исключается из
  ...
  ботов.
- Автоматически в ночной контур попадает только тематический кандидат с
  успешно открытой публичной страницей; private/empty/error, игнорируемые и
  нетематические результаты не меняют inventory.
- Разведка не записывает authoritative tier-файлы. Подходящие кандидаты
  подхватываются `nightly_discovery.py`, который уже владеет
  `public_sources.txt` и `source_catalog.txt` вместе с source-tier maintenance
  под общей concurrency-группой.
- Успешное завершение workflow разведки сразу запускает ночной discovery;
  failed/cancelled intelligence run не меняет tier-файлы. Ежедневное ночное
  расписание остаётся страховочным запуском.
...
- Health оценивает полноту intelligence по `known_sources` конкретного запуска,
  а не по inventory после downstream-маршрутизации. Добавленные этим запуском
  nightly-кандидаты подтверждаются `discovery_state` и полным transport smoke.
