...
- Непрерывность Control Center обеспечивается штатным `admin-bot.yml`: live-процесс ограничен 4,5 часами при job timeout 350 минут, после штатного завершения запускает преемника, а почасовой schedule страхует разрыв. Schedule не отменяет здоровый live-процесс; изменение `control_center_release.txt` и ручной release заменяют его. Валидатор и live-job обязаны использовать один exact SHA из release-marker; validation не переключается на `main` и не изменяет репозиторий.
