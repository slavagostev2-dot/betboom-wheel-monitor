...
class TelegramPanelRuntimeV22(TelegramPanelRuntimeV21):
    """Current panel with private Worker + D1 state storage."""
    ...
    def save_access(self, message: str = "Update Telegram panel access") -> None:
        del message  # Git commits are no longer used for personal state.
        with self.access_lock:
...
def self_test() -> None:
    ...
    try:
        ...
        private_state.save_access = lambda value: saved.append(value)  # type: ignore[assignment]
        private_state.load_source_requests = lambda default=None: {"version": 1, "requests": {}}  # type: ignore[assignment]
        private_state.save_source_requests = lambda value: saved.append(value)  # type: ignore[assignment]
        loaded = bot.load_access(force=True)
        assert loaded["owner_id"] == "1"
        bot.save_access()
        assert saved and saved[-1]["owner_id"] == "1"
        assert bot.load_source_requests()["requests"] == {}
    finally:
    ...
    print("admin_panel_runtime_v22 private state self-test passed")
...
def main() -> int:
    ...
    if not private_state.configured():
        raise SystemExit(
            "Private state API is not configured. Deploy state-api and provide BOT_TOKEN."
        )
