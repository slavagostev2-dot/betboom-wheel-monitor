...
## 2026-07-22 — Результат автоучастия больше не теряется между workflow и Control Center
...
**Post-update backup:** `backup/2026-07-22-after-seven-backup-retention` → `01d2141af1988f8915e0e1ae95b0537846bec813`.
...
## 2026-07-22 — Реферальные колёса получили явную приписку
...
Личная отметка использует существующий `mark_personal_participation` и штатную команду `record_personal_vote`: владелец даёт каждому уникальному источнику события 5 очков, actor остаётся HMAC-псевдонимом, а повтор того же события не начисляет рейтинг второй раз. После успешной отметки владельцу отправляется отдельное Telegram-уведомление «Автоучастие в колесе BetBoom подтверждено». Исторические успехи и `already_marked_participating` не создают ретроспективную рассылку. Encrypted state изменяет только Control Center; workflow автоучастия пишет публичный pending-маркер в `state.json`.

**Production-проверка:** новое колесо `zonertw5`, `action_id=685`, BetBoom подтвердил участие; event получил `bot_success_pending_at`, Control Center run `29839383473` создал личную отметку владельца и поставил стандартный `record_personal_vote` с `role=owner`, `weight=5`, источником `mechanogun`, после чего зафиксировал отправку успешного уведомления.
...
Production-доставка новых колёс теперь выполняется напрямую из живого monitor-runtime через существующий `vk_dynamic_subscribers.py`. Распознавание только первичного события, event-level dedupe и изоляция Telegram остаются в `vk_wheel_notifications.py`. В monitor workflow передаётся repository secret `VK_GROUP_TOKEN`; значение секрета не сохраняется в коде или runtime-state. Ошибка VK освобождает dedupe-claim для повторной попытки и не ломает Telegram.
...
**Post-update backup:** `backup/2026-07-21-after-direct-vk-wheel-delivery` → `126f242c5f2cc3462fd2eb6def03e0a9e1a215ca`.

**Откат:** вернуть commit целиком; миграция данных не требуется.

## 2026-07-21 — Сохранён прежний порядок кнопок аналитики
...
## 2026-07-21 — Исправлена доставка новых колёс во VK

Ночное колесо `CTOM05` было найдено и отправлено в Telegram, но отдельный
workflow VK не запускался. Диагностика подтвердила, что токен и диалоги VK
работают; сообщение отсеивалось локально до обращения к VK API, потому что
транспорт требовал один из фиксированных заголовков уведомления. Публикация
CTOM05 содержала корректную ссылку BetBoom, но имела другую формулировку.

Первичное событие теперь распознаётся по ссылке колеса или разметке кнопки, а
не по конкретному заголовку. Меню активных колёс, напоминания, уведомления о
времени прокрутки и системные ошибки по-прежнему не отправляются во VK. Все
допустимые первичные формулировки приводятся к фазе `detected`, чтобы смена
текста не создавала повторную рассылку. Добавлены regression-тест реального
формата CTOM05 и отрицательный тест системной ошибки со ссылкой.

`monitor.yml` теперь отслеживает production-модули и workflow VK, компилирует
их и запускает безопасные self-test. Новых постоянных файлов и миграции
состояния нет.
...
независимых рассинхронизаций текущего `main`: две записи safety-политики
текстовых админ-команд имели неверную длину; `ai_runtime_state.json` не был
зарегистрирован в едином ownership-контракте; workflow панели перестал явно
объявлять единственный записываемый private-state; изменения tier-файлов сами
запускали ночную разведку; UI-тест не учитывал уже добавленный экран профиля.

Контракты приведены к фактической production-архитектуре без расширения прав,
изменения ролей или миграции пользовательских данных. Ночная разведка сохраняет
право атомарно записывать `public_sources.txt` и `source_catalog.txt`, но её
собственные tier-коммиты больше не вызывают повторный запуск. Полный pytest на
итоговом дереве прошёл успешно. Новых постоянных файлов нет.
...
`alex_white_official`, `ayukifunpage`, `nucleartigeres`. Остальные 99 попали
туда двумя автоматическими пакетами разведки только по косвенной тематике.
Ни у одного из этих 99 каналов не было найдено ссылки на колесо ни в
`intelligence_state.json`, ни в `discovery_state.json`, поэтому они удалены из
ночного inventory; исторические записи разведки сохранены для аудита.

Разведка больше не запускает ночную проверку и не может пополнять ночной
список. Семидневное обслуживание источников переведено в режим аудита: оно
пишет кандидатов в `would_move_to_nightly`, но не изменяет tier-файлы. Добавить
канал в ночной режим теперь может только администратор; подтверждённое активное
колесо по-прежнему автоматически повышает уже одобренный ночной источник в
основную проверку. Health требует политику `manual_nightly_only`.

Ожидаемый inventory после обновления: 159 primary, 3 nightly, 162 всего.
Новых файлов и миграции схемы нет.

Откат: вернуть commit целиком и восстановить `source_catalog.txt` из
pre-update backup; исторические intelligence/discovery state не изменялись.

## 2026-07-19 — Источники сразу подхватываются всеми проверками

При добавлении канала через Telegram-панель список источников сохранялся, но
monitor workflow запускался без `replace`. Уже работающий шестичасовой процесс
оставался на старом inventory, а transport smoke и unified registry вообще не
запускались, потому что commit от `GITHUB_TOKEN` не создаёт новые push workflow.
Это давало ложный critical: например, `primary=157/159` и
`accounted=216/218` при доступном Telegram-транспорте.

После одиночного, массового или одобренного пользовательского добавления панель
теперь явно запускает замену монитора, транспортную проверку и перестроение
реестра. System health пересчитывается штатным `workflow_run` после завершения
этих проверок. Ошибка запуска одной проверки журналируется, но не превращает
уже успешно сохранённое изменение источника в сообщение «команда не выполнена».

Схема данных и списки источников не менялись; новых файлов нет.

Откат: вернуть commit целиком; миграция данных не требуется.

## 2026-07-18 — Восстановлены все потерянные маршруты кнопок панели

После переноса production-панели из цепочки versioned runtime в стабильные
`bbvg/bot/*` часть кнопок продолжала формироваться, но соответствующие маршруты
не были перенесены. «Интервал проверки» возвращал пользователя в меню, списки и
карточки кандидатов ночного наблюдения завершались неизвестной командой, а
кнопка активных колёс в уведомлениях не имела обработчика.

В стабильном runtime восстановлены экран выбора интервала и все значения
1/3/5/10/15/30 минут. Выбор атомарно сохраняется в зашифрованном состоянии и
запрашивает замену непрерывного monitor workflow; если GitHub временно не
принимает restart, сохранённое значение всё равно подхватывается следующим
циклом. Возвращены списки, пагинация, карточка и действия кандидатов ночного
наблюдения, включая совместимость с уже отправленными `candidate:*` callback.
`bb:l:active` снова открывает актуальный список колёс для пользователя и
администратора.

Проверены все 101 статически формируемые формы callback (включая тестовые
образцы), все доступные `page:*` ведут в собственный экран. Добавлены
поведенческие проверки интервала, прав доступа, служебных экранов настроек,
nightly-кандидатов и кнопок уведомлений. Схема данных и файлы состояния не
менялись; новых файлов нет.

Откат: вернуть commit целиком; миграция данных не требуется.

## 2026-07-18 — Исправлены кнопки администраторского уведомления разведки

Внешний `source_intelligence_alerts.py` продолжал создавать исторические
callback `candidate:mode:*`, хотя после главы 5 стабильный dispatcher панели
владеет префиксом `intel:*`. Поэтому «Добавить в источники» возвращала
«Неизвестная команда», а «Пока проигнорировать» зависела от удалённого
versioned-слоя.

Новые карточки формируют только `intel:mode:*` и `intel:defer:*`. Финальный
dispatcher обрабатывает оба действия и сохраняет compatibility-алиасы
`candidate:mode:*` и `candidate:defer:*`, поэтому кнопки в уже доставленных
сообщениях тоже остаются рабочими. Схема state, роли и списки источников
не менялись; новых файлов нет.

Откат: вернуть commit hotfix целиком; миграция данных не требуется.

## 2026-07-18 — Администраторы получают уведомление об автоматическом повышении источника

Когда ночная проверка находит в автоматически отобранном канале
подтверждённое активное колесо, канал как и раньше сразу переходит
из ночного списка в основной. Теперь владелец и администраторы дополнительно
получают служебное сообщение с username, ID колеса и ссылкой на Telegram-
публикацию. Обычным пользователям это служебное уведомление не рассылается.
Требование подтверждённого `active`-статуса и действующего таймера/кнопки
не ослаблялось; старые и неподтверждённые посты канал не повышают.
Формат state, callback и списки получателей не менялись; новых файлов нет.
