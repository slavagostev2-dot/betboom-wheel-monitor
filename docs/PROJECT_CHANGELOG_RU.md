...
- новый тематический публичный кандидат автоматически подхватывается ночным
  мониторингом; только подтверждённое активное колесо повышает его в основной
  режим;
...
- transport smoke переименован из исторического «66 источников», запускается
  при изменении любого tier-файла и после завершения повторно вызывает health;
...
**Проверки до deploy:** compile изменённых Python-модулей; 26 относящихся и
полный набор из 138 pytest-тестов + 9 subtests, включая отсечение bot/noise,
