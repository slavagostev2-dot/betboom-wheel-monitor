...
`auto-participation.yml` в `17:54 UTC`, но после принятого
`workflow_dispatch` в `auto_participation_events` не появилась ни одна
попытка, а карточка осталась `participating: false`. Workflow при этом
публиковал промежуточное и финальное состояние без результата участия.

Контракт workflow позволял любой ошибке общей проверки трёх сессий остановить
все последующие обычные шаги GitHub Actions из-за неявного `success()`.
Основная event-попытка, быстрый recovery, оба дополнительных аккаунта и полный
source-recovery теперь имеют явное `always()`-условие после выбора normal/probe
режима. Поэтому ошибка preflight или одного аккаунта сохраняет диагностический
failure запуска, но больше не препятствует попыткам остальных аккаунтов и
финальному state-merge.

Добавлен regression-контракт, который фиксирует fail-independent условие для
всех семи рабочих шагов. Callback, формат runtime-state, правила реферальных
колёс и единый Control Center не изменены.

**Pre-update backup:**
`backup/2026-07-23-before-auto-participation-preflight-isolation` →
`2d5a7915ecf6ac9a5ba49e2cb3e11f56c5096a43`.
...
Новый заголовок включён в пользовательский wheel-контракт. Dispatch автоучастия перенесён в единый post-scan путь: сначала актуальное колесо сохраняется в `state.json`, затем один state-backed dispatcher запускает workflow. Прежний ранний dispatch из notification checkpoint удалён, потому что он мог стартовать на snapshot до появления нового колеса и создавать пустой параллельный запуск.

По новому требованию реферальные колёса полностью исключены из Telegram- и VK-уведомлений, напоминаний и итогов автоучастия. Они остаются во внутреннем состоянии и активном списке; автоматические попытки участия, личная отметка и рейтинг не отключаются. Обычные колёса дедуплицируются по стабильной wheel-event identity даже при различающемся тексте источников.

Production acceptance и Control Center validator синхронизированы с действующей командой `notification_button_recovery.py`; устаревшая подмена workflow на прямой запуск `admin_panel_runtime_v41.py` удалена.

**Pre-update backup:** `backup/2026-07-23-before-notification-referral-auto-dispatch-repair` → `670e77d8645026e560d01ec32066fc7c51567748`.
...
## 2026-07-22 — Второй BetBoom-аккаунт подключён к Вячеславу

Добавлена независимая сессия из `BETBOOM_STORAGE_STATE_JSON_PART3` и `BETBOOM_STORAGE_STATE_JSON_PART4`. Для неё используется отдельный account event-token `vyacheslav_secondary`, поэтому оба BetBoom-аккаунта обрабатывают одно колесо независимо и не перезаписывают результаты друг друга.

Успех или подтверждённая терминальная ошибка второго аккаунта финализируются единственным Control Center и отправляются Telegram-пользователю «Вячеслав» с пометкой «Аккаунт 2». Повторное начисление рейтинга одному Telegram-пользователю за одно событие исключено штатной идемпотентностью личного голоса.

**Pre-update backup:** `backup/2026-07-22-before-vyacheslav-second-account`.

## 2026-07-22 — Уведомления участия и AI-диагностика пересобраны

Пользовательский исход автоучастия теперь принадлежит только единственному живому Control Center. Прямой legacy-failure запрещён; `unconfirmed`, browser/network timeout и dispatch-сбои остаются бесшумным повторяемым состоянием. Успех и подтверждённая терминальная неудача показываются коротко, с идентификатором колеса и только кнопками «Активные колёса»/«Главное меню». Все уведомления о колёсах получили переход к активному списку.

AI-анализ публикаций допускает модель только при проверяемом сочетании колеса, BetBoom и действия, а итоговая уверенность ограничивается признаками исходного текста. Абстрактное упоминание стрима не считается сигналом. Переменный текст AI-инспектора исключён из Telegram-идентичности системного инцидента; push-запуски не рассылают временные ошибки развёртывания.

Из раздела «Источники» удалена кнопка «Обновить реестр», из аналитики скрыт недостоверный блок «Топ источников по находкам». Security-аудит валидирует HMAC-карту message checkpoint, исправлены immutable SHA `actions/setup-python` в backup workflow.

**Pre-update backup:** `backup/2026-07-22-before-notification-ai-diagnostics-rebuild`.

**Pre-merge validation:** `Validate current` run `29913715221`, `current checks` run `29913715118`, recovery smoke `29913715242`, bot-only `29913715161`, Telegram transport `29913715124` — успешно.
